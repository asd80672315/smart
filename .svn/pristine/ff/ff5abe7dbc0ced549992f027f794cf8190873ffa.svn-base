...
public class AppUtil  {
	...
	public static boolean checkParam(String method, PageData pd){
		...
		if(method=="registered"){// 注册
			paramArray = new String[]{"countries","uname","passwd","title","full_name","company_name","countries_code","area_code","telephone","mobile"};  //参数
			valueArray = new String[]{"国籍","邮箱帐号","密码","称谓","名称","公司名称","国家编号","区号","电话","手机号"};  //参数名称
			
		}else if(method=="getAppuserByUsernmae"){//根据用户名获取会员信息
			paramArray = new String[]{"USERNAME"};  
			valueArray = new String[]{"用户名"};
		}
